...
trait RunOperation {
  ...
  def run(implicit db: Kuzminki): Future[Unit] =
    db.exec(render)

  def runNum(implicit db: Kuzminki): Future[Int] =
    db.execNum(render)
}
...
trait RunOperationParams[P] {
  ...
  def run(params: P)(implicit db: Kuzminki): Future[Unit] =
    db.exec(render(params))

  def runNum(params: P)(implicit db: Kuzminki): Future[Int] =
