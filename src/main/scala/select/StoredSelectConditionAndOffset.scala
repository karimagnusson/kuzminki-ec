...
class StoredSelectConditionAndOffset[P, R](
  ...
  def run(params: P, offset: Int)(implicit db: Kuzminki): Future[List[R]] =
    db.query(render(params, offset))

  def runHead(params: P, offset: Int)(implicit db: Kuzminki): Future[R] =
    db.queryHead(render(params, offset))

  def runHeadOpt(params: P, offset: Int)(implicit db: Kuzminki): Future[Option[R]] =
